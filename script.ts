...
function saveDataAndRedirect(): void {
    ...
    if (photoInput.files && photoInput.files[0]) {
        ...
        reader.onload = () => {
            ...
            const resumeData: ResumeData = { name, email, phone, summary, education, experience, skills, projects, achievements,certifications,languages,references, photo };
            localStorage.setItem("resumeData", JSON.stringify(resumeData));
            window.location.href = `resume.html?id=${Date.now()}`;
        };
        reader.readAsDataURL(photoInput.files[0]);
    } else {
        const resumeData: ResumeData = { name, email, phone, summary, education, experience, skills, projects, achievements,certifications,languages,references };
        localStorage.setItem("resumeData", JSON.stringify(resumeData));
        window.location.href = `resume.html?id=${Date.now()}`;
    }
}